"""
Variant v8_optimal: Our RECOMMENDED BEST model.
10L/384d, ~32M params, 650M pretraining tokens (20:1 optimal ratio).
Adds 2 more layers to v7's architecture — deeper processing for claim+evidence.
Purpose: Test whether additional depth with optimal token ratio beats v7.
Hypothesis: 20:1 Chinchilla-optimal ratio + 2 extra layers should maximize accuracy
  while staying within T4 VRAM budget (batch=16 at 384d fits 16GB).
Time on T4: ~6h pretraining + 0.5h fine-tuning = ~6.5h total
"""
...
# ═══ V8: 10 layers, 384d — Chinchilla-optimal at 20:1 ═══
CONFIG = ModelConfig(d_model=384, n_layers=10, n_heads=6, n_kv_heads=2, max_seq_len=1024)
NUM_TOKENS = 650_000_000   # 20:1 token:param ratio — Chinchilla optimal
BATCH_SIZE = 16
LR = 3e-4
LABEL = "v8_optimal"
...
class Tokenizer:
    ...
    def __init__(self, model_path=None):
        if model_path and os.path.exists(model_path):
            ...
            spm.SentencePieceTrainer.train(input=f.name, model_prefix='/tmp/sp_v8', vocab_size=8000,
                model_type='bpe', pad_id=0, unk_id=1, bos_id=2, eos_id=3, user_defined_symbols=['<sep>'])
            self.sp = spm.SentencePieceProcessor(model_file='/tmp/sp_v8.model')
            os.unlink(f.name)
...
print(f"[{LABEL}] Token:Param ratio: {NUM_TOKENS/n_params:.1f}:1 (target: 20:1)")
print(f"[{LABEL}] Pretraining {NUM_TOKENS:,} tokens, {total_steps} steps")
