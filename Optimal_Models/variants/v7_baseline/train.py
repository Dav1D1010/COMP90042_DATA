"""
Variant v7_baseline: Original winning architecture (8L/384d, 25M params)
Trains on 150M tokens — reproduces our experiment baseline.
Purpose: Confirm that the architecture works, establish baseline score.
Expected: ~42% accuracy (our experiment result)
Time on T4: ~1.5h pretraining + 0.5h fine-tuning = ~2h total
"""
...
# Config — Original v7
# ═══════════════════════════════════════════════════════════════════════════
CONFIG = ModelConfig(d_model=384, n_layers=8, n_heads=6, n_kv_heads=2, max_seq_len=1024)
NUM_TOKENS = 150_000_000   # Same as experiment baseline
BATCH_SIZE = 16
LR = 3e-4
LABEL = "v7_baseline"
...
class Tokenizer:
    ...
    def __init__(self, model_path=None):
        if model_path and os.path.exists(model_path):
            ...
            spm.SentencePieceTrainer.train(input=f.name, model_prefix='/tmp/sp_v7', vocab_size=8000,
                model_type='bpe', pad_id=0, unk_id=1, bos_id=2, eos_id=3, user_defined_symbols=['<sep>'])
            self.sp = spm.SentencePieceProcessor(model_file='/tmp/sp_v7.model')
            os.unlink(f.name)
...
print(f"[{LABEL}] Pretraining {NUM_TOKENS:,} tokens, {total_steps} steps, warmup={warmup}")
...
for batch in loader:
    ...
    if step % 100 == 0:
        ...
        eta = (NUM_TOKENS-tokens)/speed if speed>0 else 0
        print(f"  Step {step:>6d} | {100*tokens/NUM_TOKENS:5.1f}% | loss={running_loss/100:.4f} | {speed:,.0f} tok/s | ETA={timedelta(seconds=int(eta))}")
        running_loss = 0
    if tokens >= NUM_TOKENS: break

pt_path = f"{LABEL}_pretrain.pt"
torch.save(model.state_dict(), pt_path)
elapsed = time.time() - t0
print(f"[{LABEL}] Pretraining done: {tokens:,} tokens in {timedelta(seconds=int(elapsed))} ({tokens/elapsed:,.0f} tok/s)")

# ═══════════════════════════════════════════════════════════════════════════
# Fine-tuning (NO DRW — stable LDAM+CB+label smoothing from epoch 1)
# ═══════════════════════════════════════════════════════════════════════════
class_counts = torch.tensor([519., 199., 386., 124.])  # SUPPORTS, REFUTES, NOT_ENOUGH, DISPUTED
...
class ClfDS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        cid,d = self.items[i]; return {"claim_id":cid,"claim_text":d["claim_text"],"claim_label":d.get("claim_label"),"evidences":d.get("evidences",[])}

c_train = DataLoader(ClfDS(train_data), batch_size=4, shuffle=True,
    collate_fn=ClfCollator(tokenizer, evidence))
c_dev = DataLoader(ClfDS(dev_data), batch_size=4, shuffle=False,
    collate_fn=ClfCollator(tokenizer, evidence))
...
for epoch in range(5):
    ...
    with torch.no_grad():
        for batch in c_dev:
            ids=batch["input_ids"].to(device); am=batch["attention_mask"].to(device)
            targets=batch["labels"].to(device)
            preds=classifier(ids,am).argmax(-1)
