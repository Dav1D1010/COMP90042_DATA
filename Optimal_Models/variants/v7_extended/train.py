"""
Variant v7_extended: Same architecture as original v7 (8L/384d, 25M params)
BUT trained on 800M tokens instead of 150M.
Purpose: Test whether more pretraining tokens directly improves accuracy.
Hypothesis: With 800M/25M = 32:1 ratio (above 20:1 optimal), 
  performance should significantly exceed the baseline 42%.
Time on T4: ~7.5h pretraining + 0.5h fine-tuning = ~8h total
"""
...
NUM_TOKENS = 800_000_000   # 32:1 token:param ratio — above Chinchilla optimal
BATCH_SIZE = 16
LR = 3e-4
LABEL = "v7_extended"
...
class Tokenizer:
    ...
    def __init__(self, model_path=None):
        if model_path and os.path.exists(model_path):
            ...
            spm.SentencePieceTrainer.train(input=f.name, model_prefix='/tmp/sp_v7e', vocab_size=8000,
                model_type='bpe', pad_id=0, unk_id=1, bos_id=2, eos_id=3, user_defined_symbols=['<sep>'])
            self.sp = spm.SentencePieceProcessor(model_file='/tmp/sp_v7e.model')
            os.unlink(f.name)
...
print(f"[{LABEL}] Pretraining {NUM_TOKENS:,} tokens (ratio: {NUM_TOKENS/n_params:.1f}:1)")
print(f"[{LABEL}] Steps: {total_steps}, warmup: {warmup}")
